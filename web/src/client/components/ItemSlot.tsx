        ...
        <div className="cursor-pointer rounded border border-border hover:border-primary transition-colors bg-card">
          ...
          <div className="px-2 pt-1 pb-0.5">
            ...
            {item ? (
              <>
                <div
                  className={`text-xs font-medium leading-tight ${RARITY_COLORS[item.rarity] ?? ""}`}
                >
                  {item.name || item.base}
                </div>
                {item.name && item.base && (
              ...
              <div className="text-[10px] text-muted-foreground/50 italic leading-tight">
                空
              </div>
            )}
          </div>
            ...
            (mods.implicits.length > 0 || mods.explicits.length > 0) && (
              <div className="px-2 pb-1 pt-0.5 border-t border-border/30">
                {mods.implicits.map((mod, i) => (
                  <div
                    key={`imp-${i}`}
                    className="text-[10px] text-neutral-300/80 leading-[1.3]"
                  >
                    {mod}
                  </div>
                ))}
                ...
                {mods.explicits.map((mod, i) => (
                  <div
                    key={`exp-${i}`}
                    className={`text-[10px] ${modColor} leading-[1.3]`}
                  >
                    {mod}
                  </div>
                ))}
